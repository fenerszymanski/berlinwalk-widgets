...
const BW_WIDGETS_HUB_DATA_URL = 'https://fenerszymanski.github.io/berlinwalk-widgets/tools-hub/data.json?v=20260831-featured-listings';
...
class BWWidgetsHubElement extends HTMLElement {
  ...
  _renderShell() {
    this.innerHTML = `
      <style>
        ...
        .bw-widgets-hub .bw-start-here h2 {
          color: #FFE600;
          font-size: 22px;
          font-weight: 800;
          line-height: 1.2;
        ...
        .bw-widgets-hub .bw-start-grid {
          display: grid;
          gap: 10px;
          grid-template-columns: repeat(3, minmax(0, 1fr));
        }

        .bw-widgets-hub .bw-start-card {
          align-items: center;
          background: #FFFFFF;
        ...
        .bw-widgets-hub .bw-start-card:hover,
        .bw-widgets-hub .bw-start-card:focus-visible {
          background: #FFE600;
          color: #123D18;
          outline: none;
        }

        .bw-widgets-hub .bw-start-card img {
          border-radius: 7px;
          flex: 0 0 32px;
          height: 32px;
          object-fit: cover;
        ...
        .bw-widgets-hub .bw-start-card span {
          min-width: 0;
        ...
        .bw-widgets-hub .bw-preview-panel[hidden] { display: none; }
        ...
        .bw-widgets-hub .bw-preview-toggle,
        ...
        .bw-widgets-hub .bw-preview-toggle:hover,
        .bw-widgets-hub .bw-preview-toggle:focus-visible {
          background: #fff04a;
          outline: 2px solid #123D18;
          outline-offset: 2px;
        }
        ...
        @media (max-width: 700px) {
          ...
          .bw-widgets-hub .bw-start-here { padding: 18px; }
          .bw-widgets-hub .bw-start-grid { grid-template-columns: 1fr; }
  ...
  _renderHub(data) {
    ...
    const startHere = this._renderStartHere(tools);
    const sections = categories
      .map(category => this._renderCategory(category, tools))
      .filter(Boolean)
      .join('');
    root.innerHTML = nav + startHere + sections;
  }

  _renderStartHere(tools) {
    ...
    const cards = featured.map(tool => {
      ...
      return `
        <a class="bw-start-card" href="#bw-widget-${slug}">
          ${image ? `<img src="${image}" alt="" loading="lazy" decoding="async">` : ''}
          <span>${title}</span>
        </a>`;
    }).join('');
    return `
      <section class="bw-start-here" aria-labelledby="bw-start-here-title">
        <h2 id="bw-start-here-title">Start here</h2>
        <p class="bw-start-lead">Six useful first stops. Every other widget stays easy to scan below.</p>
        <div class="bw-start-grid">${cards}</div>
      </section>`;
  }
  ...
  _renderWidgetCard(tool) {
    ...
    const themePills = Object.keys(BW_WIDGETS_THEMES)
      ...
      .join('');
    return `
      <article class="bw-widget-card" id="bw-widget-${slug}" data-slug="${slug}">
        <div class="bw-widget-head">
          <div class="bw-widget-head-row">
            ...
            <div class="bw-widget-meta">Preview on demand · ${this._escapeHtml(tool.category || '')}</div>
          </div>
        </div>
        <div class="bw-preview-panel" hidden data-preview-panel-slug="${slug}">
          <div class="bw-preview-hint">Live preview</div>
          <div class="bw-preview-frame" data-preview-frame-slug="${slug}" data-preview-url="${previewUrl}" data-preview-title="${this._escapeAttribute(tool.title || '')}"></div>
        </div>
        <div class="bw-widget-actions">
          <button class="bw-preview-toggle" type="button" data-preview-slug="${slug}" aria-expanded="false">Preview</button>
          <button class="bw-embed-toggle" type="button" data-toggle-slug="${slug}" aria-expanded="false">Get embed code</button>
...
<script src="${BW_EMBED_RESIZE_URL}" async></` + `script>`;
  ...
  _bindGlobalHandlers() {
    this.addEventListener('click', (ev) => {
      const previewToggle = ev.target.closest('[data-preview-slug]');
      if (previewToggle) {
        const slug = previewToggle.getAttribute('data-preview-slug');
        const card = previewToggle.closest('.bw-widget-card');
        const section = card && card.closest('.bw-category-section');
        const panel = card && card.querySelector('[data-preview-panel-slug]');
        if (!panel || !section) return;
        const opening = panel.hidden;

        section.querySelectorAll('[data-preview-panel-slug]').forEach(otherPanel => {
          if (otherPanel === panel) return;
          otherPanel.hidden = true;
          const otherCard = otherPanel.closest('.bw-widget-card');
          const otherButton = otherCard && otherCard.querySelector('[data-preview-slug]');
          if (otherButton) {
            otherButton.setAttribute('aria-expanded', 'false');
            otherButton.textContent = 'Preview';
          }
          const otherFrame = otherPanel.querySelector('iframe[data-bw-frame]');
          if (otherFrame) otherFrame.remove();
        });

        panel.hidden = !opening;
        previewToggle.setAttribute('aria-expanded', String(opening));
        previewToggle.textContent = opening ? 'Close preview' : 'Preview';
        const frameHost = panel.querySelector('[data-preview-frame-slug]');
        if (opening && frameHost && !frameHost.querySelector('iframe')) {
          const frame = document.createElement('iframe');
          frame.setAttribute('data-bw-frame', '');
          frame.src = frameHost.getAttribute('data-preview-url') || '';
          frame.title = (frameHost.getAttribute('data-preview-title') || slug) + ' live preview';
          frame.loading = 'eager';
          frame.setAttribute('frameborder', '0');
          frame.setAttribute('scrolling', 'no');
          frame.height = '420';
          frame.style.cssText = 'width:100%; border:0; transition:height .2s ease;';
          frameHost.appendChild(frame);
        } else if (!opening && frameHost) {
          const frame = frameHost.querySelector('iframe[data-bw-frame]');
      ...
      if (toggle) {
        ...
        toggle.textContent = open ? 'Get embed code' : 'Hide embed code';
        return;
      }
