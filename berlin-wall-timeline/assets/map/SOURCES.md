...
- Berlin Ortsteile: [Berlin Geoportal WFS via ODIS WFS Explorer](https://wfsexplorer.odis-berlin.de/?layer=alkis_ortsteile%3Aortsteile&wfs=https%3A%2F%2Fgdi.berlin.de%2Fservices%2Fwfs%2Falkis_ortsteile). Ortsteile are dissolved into historical occupation-sector overlays for story use; individual district outlines and labels are intentionally not rendered.
- Waterways and named water areas: [OpenStreetMap](https://www.openstreetmap.org/copyright), queried from the Berlin bounding box through the Overpass API and simplified to the major named waterways, canals and lakes retained in the reproducible data package. The timeline does not render this optional layer so the sector scene stays visually clean.
