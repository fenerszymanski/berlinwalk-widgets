...
function bwGdprDefaultPolicyBlocked(current) {
...
function bwAdvertisingConsentAllowed() {
  try {
    ...
    const current = manager && typeof manager.getCurrentConsentPolicy === 'function'
      ...
      : null;
    if (bwGdprDefaultPolicyBlocked(current)) return false;
    const policy = current && (current.policy || current);
    const source = policy && Object.keys(policy).length
      ? policy
      : (() => {
    ...
    const nested = source.consent && typeof source.consent === 'object' ? source.consent : {};
    return bwConsentBoolean(source.advertising ?? source.adv ?? source.advertisingConsent ?? source.marketing ?? source.marketingConsent ?? source.dataToThirdParty ?? source.marketingStorage ?? nested.advertising ?? nested.marketing);
  } catch {
    return false;
  }
}
...
class BWBookingCalendarElement extends HTMLElement {
  ...
  _bookingHref(slot) {
    ...
    const advertisingAllowed = bwAdvertisingConsentAllowed();
    ['utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_term', 'utm_id'].forEach((param) => {
    ...
    ['fbclid', 'fbc', 'fbp'].forEach((param) => {
      if (advertisingAllowed && incoming.searchParams.has(param)) url.searchParams.set(param, incoming.searchParams.get(param));
      else if (!advertisingAllowed) url.searchParams.delete(param);
    });
...
if (window.BW_BOOKING_CALENDAR_TEST_HOOKS) {
