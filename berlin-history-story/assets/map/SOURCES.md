...
`map-data.json` is a compact, pre-projected SVG-path package copied from the verified Wall Timeline map source and relabelled for this History Story package. It is not a live tile map and makes no runtime request to a map provider beyond loading this static package.
