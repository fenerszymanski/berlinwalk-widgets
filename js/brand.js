...
(function () {
  ...
  // A widget nested inside a Wix sandbox reaches the site's resize listener only
  // through window.top, and the listener can only match it back to an iframe by
  // URL because contentWindow !== event.source there. A page opts into that
  // delivery with window.__bwResizePostToTop = true before this script runs; the
  // default single post to window.parent is unchanged for every other widget.
  function postResize(height) {
    var message = { type: 'bw-resize', height: height };
    if (!window.__bwResizePostToTop) {
      try { window.parent.postMessage(message, '*'); } catch (e) { /* parent unreachable, ignore */ }
      return;
    }
    message.src = window.location.pathname + window.location.search;
    var posted = false;
    try {
      if (window.top && window.top !== window) { window.top.postMessage(message, '*'); posted = true; }
    } catch (e) { /* cross-origin top, fall through to parent */ }
    try {
      if (!posted || (window.parent && window.parent !== window && window.parent !== window.top)) {
  ...
  function reportThrottled() {
    ...
    // A cross-origin iframe below the fold has its rAF callbacks frozen. Without
    // this fallback the first report after content arrives never flushes, the
    // throttle stays latched, and every later trigger returns early too, so the
    // widget sits at the host's default box until the reader scrolls to it.
...
(function () {
  ...
  try {
    isQuickSummary = document.documentElement.getAttribute('data-bw-component') === 'quick-summary'
      || /(?:^|\/)quick-summary(?:\/|$)/.test(window.location.pathname || '');
  } catch (e) { /* missing marker/path, keep the global widget contract */ }

  // Redesign C owns this summary's conversion surface in the parent article
  // rail, so its compact tile strip deliberately has no iframe footer CTA.
  var isRedesignCQuickSummary = !!(isQuickSummary && params && params.get('bwredesign') === '1');
  if (isRedesignCQuickSummary) return;
  ...
  function injectBadgeOnly() {
    // Quick Summary already carries its own BERLINWALK.COM line. Do not add a
    // second, green global attribution surface when no tour slot is available.
    if (isQuickSummary) return;
  ...
  function renderFirstPartyTourCta(slug, text) {
    ...
    if (existing) {
      if (isQuickSummary) {
        existing.classList.add('bw-tour-cta-row--full');
        Array.prototype.forEach.call(existing.querySelectorAll('.bw-attr-badge'), function (badge) { badge.remove(); });
      }
    ...
    row.className = 'bw-tour-cta-row' + (isQuickSummary ? ' bw-tour-cta-row--full' : '');
    row.innerHTML =
