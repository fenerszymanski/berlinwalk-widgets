...
(function () {
  ...
  var BLOG_JOURNEY_TEST_MODE = Boolean(window.BW_BLOG_JOURNEY_TEST_HOOKS);

  // Redesign C owns the entire post-only layer when its explicit preview flag
  // is present. Keep this gate before legacy rendering so the old share bar,
  // mobile navigation and end module are never added to the redesign DOM.
  function redesignEnabled() {
    try {
      return window.location.search.indexOf('bwredesign=1') !== -1 || window.localStorage.getItem('bwRedesign') === '1' || window.BW_REDESIGN_DEFAULT_ON === true;
    } catch (error) { return window.BW_REDESIGN_DEFAULT_ON === true; }
  }

  function startRedesignC() {
    if (document.querySelector('script[data-bw-redesign-c-loader]')) return;
    var script = document.createElement('script');
    script.async = true;
    script.setAttribute('data-bw-redesign-c-loader', '1');
    script.src = resolveAdjacentScriptUrl('blog-redesign-c-inject.js?v=blog-redesign-c-20260823');
    document.head.appendChild(script);
  }

  function blogEngagementRequested() {
    try {
      var override = new URLSearchParams(window.location.search || '').get('bwBlogEngagement');
      if (override === '0') return false;
      return override === 'control' || override === 'utility' || (window.BW_BLOG_ENGAGEMENT_CONFIG && window.BW_BLOG_ENGAGEMENT_CONFIG.enabled === true);
    } catch (error) {
      return Boolean(window.BW_BLOG_ENGAGEMENT_CONFIG && window.BW_BLOG_ENGAGEMENT_CONFIG.enabled === true);
    }
  }

  function startBlogEngagementPilot() {
    if (!isPostPage() || !blogEngagementRequested() || document.querySelector('script[data-bw-blog-engagement-loader]')) return;
    var script = document.createElement('script');
    script.async = true;
    script.setAttribute('data-bw-blog-engagement-loader', '1');
    script.src = resolveAdjacentScriptUrl('blog-engagement-pilot.js?v=20260824a');
    document.head.appendChild(script);
  }
  ...
  function installDelayedConsentGuard() {
    ...
    var firstPartyEventRe = /berlinwalk-content-app\.vercel\.app\/api\/(?:pf-event|tp-event|blog-engagement)/i;
    var metaEventRe = /(?:facebook\.com\/tr|connect\.facebook\.net|\/_serverless\/analytics-reporter\/facebook\/event)/i;

    function policy() {
      return currentConsentPolicy();
    }

    function hasConsent(kind) {
      var current = policy();
      var state = currentConsentState();
      if (kind === 'analytics') return state.analytics;
      if (kind === 'advertising') return state.advertising;
      if (kind === 'functional') return current.functional === true || current.func === true || current.func === 1;
      return false;
    }

    function consentKnown(kind) {
      var current = policy();
      if (!current || !Object.keys(current).length) return false;
      var state = currentConsentState();
      if (kind === 'analytics') return state.analyticsKnown;
      if (kind === 'advertising') return state.advertisingKnown;
      if (kind === 'functional') return current.functional !== undefined || current.func !== undefined;
      return false;
    }
    ...
    function guardStorage(storage, keyMap, consentKind) {
      ...
      function clearBlockedKeys() {
        if (!consentKnown(consentKind) || hasConsent(consentKind)) return;
  ...
  function installConsentGatedBookingAnalytics() {
    ...
    function initTracking() {
      return canonicalPaidTrackingState();
    }

    function ensureTracking() {
      if (!tracking || !currentConsentState().advertising) tracking = initTracking();
      return tracking;
    }
    ...
    function sendEndpoint(name, payload, state) {
      ...
      var eventId = payload && payload.event_id || trackingRandomId('bw_e');
      var body = JSON.stringify({
        ...
        pageUrl: privacySafeTrackingUrl(window.location.href, consent.advertising),
        ...
        utmId: state.utm_id || '',
        fbclid: consent.advertising ? state.fbclid || '' : '',
        fbc: consent.advertising ? state.fbc || '' : '',
        fbp: consent.advertising ? state.fbp || '' : '',
    ...
    function sendEvent(name, detail) {
      ...
      var payload = Object.assign({
        ...
        utm_source: state && state.utm_source || params.get('utm_source') || '',
        utm_medium: state && state.utm_medium || params.get('utm_medium') || '',
        utm_campaign: state && state.utm_campaign || params.get('utm_campaign') || '',
        event_id: trackingRandomId('bw_e'),
        attribution_id: state && state.attributionId || '',
      }, detail || {});
    ...
    function once(key, name, detail) {
      var previous = sent[key] || { analytics: false, advertising: false };
      var result = sendEvent(name, detail, {
        analytics: !previous.analytics,
        advertising: !previous.advertising,
      });
      if (result.analytics || result.advertising) {
        sent[key] = {
          analytics: previous.analytics || result.analytics,
          advertising: previous.advertising || result.advertising,
        };
      }
    }
    ...
    document.addEventListener('TagManagerConfigSet', sendInitialBookingEvents);
    document.addEventListener('ucConsentEvent', sendInitialBookingEvents);
    document.addEventListener('bwConsentPolicyChanged', sendInitialBookingEvents);
    window.addEventListener('consentPolicyChanged', sendInitialBookingEvents);
    window.addEventListener('consentPolicyInitialized', sendInitialBookingEvents);
    window.addEventListener('TagManagerConfigSet', sendInitialBookingEvents);
    window.addEventListener('ucConsentEvent', sendInitialBookingEvents);
    window.addEventListener('bwConsentPolicyChanged', sendInitialBookingEvents);
  ...
  function installConsentSettingsUi() {
    ...
    function clearTrackingState(next) {
      try {
        if (!next.analytics) {
          ['bwPaidTracking.v1', 'bwVisitorId.v1', 'bwSessionId.v1', 'bwTripPlannerVisitorId.v1', 'bwTripPlannerSessionId.v1', 'bwBlogEngagement.v1'].forEach(function (key) {
  ...
  function injectStyle() {
    ...
    style.textContent = [
      ...
      '.bw-blog-mobile-nav-top{align-items:center;display:flex;gap:12px;justify-content:space-between;margin:0 0 13px;}',
      '.bw-blog-mobile-nav-brand{align-items:center;color:#1B5E20;display:inline-flex;font-size:10px;font-weight:800;gap:7px;letter-spacing:1.1px;line-height:1.3;min-width:0;overflow:hidden;text-overflow:ellipsis;text-transform:uppercase;white-space:nowrap;}',
      '.bw-blog-mobile-nav-brand:before{background:#FFE600;border-radius:50%;box-shadow:0 0 0 2px rgba(27,94,32,.18);content:"";display:block;flex:0 0 auto;height:8px;width:8px;}',
      '.bw-blog-mobile-nav-home{align-items:center;background:#F1F7EC;border:1px solid #DCE8D4;border-radius:999px;color:#1B5E20!important;display:inline-flex;flex:0 0 auto;font-size:11.5px;font-weight:800;gap:5px;line-height:1;min-height:31px;padding:0 12px;text-decoration:none!important;white-space:nowrap;}',
      '.bw-blog-mobile-nav-home-icon{flex:0 0 auto;height:11px;width:11px;}',
      '.bw-blog-mobile-nav-scroll{margin:0;overflow:hidden;position:relative;}',
      '.bw-blog-mobile-nav-scroll:after{background:linear-gradient(90deg,rgba(250,250,245,0),#FAFAF5 85%);content:"";display:block;height:100%;pointer-events:none;position:absolute;right:0;top:0;width:30px;}',
      '.bw-blog-mobile-nav-list{display:flex;gap:7px;list-style:none;margin:0;overflow-x:auto;padding:0 30px 2px 0;scrollbar-width:none;}',
      '.bw-blog-mobile-nav-list::-webkit-scrollbar{display:none;}',
      '.bw-blog-mobile-nav-list a{align-items:center;background:#FFFFFF;border:1px solid #E3E8DC;border-radius:999px;color:#3d4a3d!important;display:inline-flex;font-size:12px;font-weight:700;line-height:1;min-height:33px;padding:0 14px;text-decoration:none!important;white-space:nowrap;}',
      '.bw-blog-mobile-nav-list a.bw-blog-mobile-nav-active{background:#1B5E20;border-color:#1B5E20;box-shadow:0 4px 10px rgba(27,94,32,.22);color:#FFFFFF!important;font-weight:800;}',
      ...
      'body.bw-blog-post-enhanced [data-bw-blog-post-body="1"] .bw-blog-mobile-nav .bw-blog-mobile-nav-home{color:#1B5E20!important;}',
      'body.bw-blog-post-enhanced [data-bw-blog-post-body="1"] .bw-blog-mobile-nav-list a{color:#3d4a3d!important;}',
      'body.bw-blog-post-enhanced [data-bw-blog-post-body="1"] .bw-blog-mobile-nav-list a.bw-blog-mobile-nav-active{color:#FFFFFF!important;}',
  ...
  function mobileBlogNavHtml(active) {
    return '<div class="bw-blog-mobile-nav-top">' +
      '<span class="bw-blog-mobile-nav-brand">Berlin Travel &amp; History Notes</span>' +
      '<a class="bw-blog-mobile-nav-home" href="https://www.berlinwalk.com/blog" target="_top">' +
      '<svg class="bw-blog-mobile-nav-home-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M19 12H5"/><path d="M12 19l-7-7 7-7"/></svg>' +
      'Blog Home</a>' +
      '</div>' +
  ...
  function currentConsentPolicy() {
    try {
      ...
      if (current && current.defaultPolicy === true) {
        ...
        if (!config || config.gdprEnforcedGeo !== false) return { analytics: false, advertising: false };
      }
    ...
    try {
      ...
      var policyFromCookie = parsed && (parsed.policy || parsed);
      return policyFromCookie && Object.keys(policyFromCookie).length ? policyFromCookie : {};
    } catch (err) {
      return {};
    }
  }
  ...
  function currentConsentState() {
    ...
    var known = Boolean(policy && Object.keys(policy).length);
    var nested = policy && policy.consent && typeof policy.consent === 'object' ? policy.consent : {};
    var analyticsRaw = policy && (policy.analytics ?? policy.anl ?? policy.analyticsConsent ?? policy.analyticsStorage ?? nested.analytics);
    var advertisingRaw = policy && (policy.advertising ?? policy.adv ?? policy.advertisingConsent ?? policy.marketing ?? policy.marketingConsent ?? policy.dataToThirdParty ?? policy.marketingStorage ?? nested.advertising ?? nested.marketing);
    var analyticsKnown = analyticsRaw !== undefined && analyticsRaw !== null;
    var advertisingKnown = advertisingRaw !== undefined && advertisingRaw !== null;
    return {
      known: known,
      analyticsKnown: analyticsKnown,
      advertisingKnown: advertisingKnown,
      analytics: analyticsKnown && consentValue(analyticsRaw),
      advertising: advertisingKnown && consentValue(advertisingRaw),
    };
  }
  ...
  function readTrackingJSON(storage, key) {
    try {
  ...
  function writeTrackingJSON(storage, key, value) {
    try {
  ...
  function privacySafeTrackingUrl(value, advertisingAllowed) {
    ...
    try {
      ...
      ['fbclid', 'fbc', 'fbp'].forEach(function (key) { url.searchParams.delete(key); });
      return url.toString();
    } catch (err) {
      return raw;
    }
  ...
  function purgePaidTrackingState(preserveAnalyticsState) {
    try {
      if (preserveAnalyticsState) {
        var stored = readTrackingJSON(window.localStorage, 'bwPaidTracking.v1');
        if (stored && typeof stored === 'object') {
          delete stored.fbclid;
          delete stored.fbc;
          delete stored.fbp;
          writeTrackingJSON(window.localStorage, 'bwPaidTracking.v1', stored);
        }
      } else {
        window.localStorage.removeItem('bwPaidTracking.v1');
        window.localStorage.removeItem('bwVisitorId.v1');
        window.sessionStorage.removeItem('bwSessionId.v1');
      }
    } catch (err) {}
  }

  function canonicalPaidTrackingState() {
    var consent = currentConsentState();
    if (!consent.analytics) {
      if (consent.analyticsKnown) purgePaidTrackingState(false);
      return null;
    }

    var params = new URLSearchParams(window.location.search || '');
    var stored = readTrackingJSON(window.localStorage, 'bwPaidTracking.v1') || {};
    var storedAdvertisingIdentifiers = ['fbclid', 'fbc', 'fbp'].some(function (key) {
    ...
    if (!merged.visitorId) merged.visitorId = readTrackingJSON(window.localStorage, 'bwVisitorId.v1') || trackingRandomId('bw_v');
    writeTrackingJSON(window.localStorage, 'bwVisitorId.v1', merged.visitorId);
    if (!merged.sessionId) merged.sessionId = readTrackingJSON(window.sessionStorage, 'bwSessionId.v1') || trackingRandomId('bw_s');
    writeTrackingJSON(window.sessionStorage, 'bwSessionId.v1', merged.sessionId);
    if (!merged.attributionId) merged.attributionId = trackingRandomId('bw_attr');
    ...
    if (consent.advertising) {
      var identifiers = {
        ...
        fbc: params.get('fbc') || readTrackingCookie('_fbc'),
        fbp: params.get('fbp') || readTrackingCookie('_fbp'),
      };
      ['fbclid', 'fbc', 'fbp'].forEach(function (key) {
    ...
    var paidSignal = [merged.utm_source, merged.utm_medium, merged.utm_campaign].join(' ').toLowerCase();
    merged.isPaid = /(^|[^a-z])(meta|facebook|instagram|fb|ig|paid|cpc|paid_social)([^a-z]|$)/i.test(paidSignal);
    merged.updatedAt = new Date().toISOString();
    // Do not overwrite an existing record with a sanitized value while Wix is
    // still initializing advertising consent; a new analytics-only record is
    // safe to persist because it has no advertising identifiers.
    if (consent.advertising || consent.advertisingKnown || !storedAdvertisingIdentifiers) {
      writeTrackingJSON(window.localStorage, 'bwPaidTracking.v1', merged);
    }
    return merged;
  }
  ...
  function advertisingConsent() {
    return currentConsentState().advertising;
  }
  ...
  function applyBookAttribution(url, context, options) {
    options = options || {};
    var incoming = new URL(window.location.href);
    var allowAdvertising = advertisingConsent();
    ATTRIBUTION_KEYS.forEach(function (key) {
      if (!allowAdvertising && (key === 'fbclid' || key === 'fbc' || key === 'fbp')) return;
  ...
  function trackPaidEvent(eventName, payload) {
    ...
    var consent = currentConsentState();
    var allowAnalytics = consent.analytics;
    var allowAdvertising = consent.advertising;
    if (!allowAnalytics && !allowAdvertising) return false;
    var state = allowAnalytics ? canonicalPaidTrackingState() : null;
    var eventPayload = Object.assign({
      page_path: window.location.pathname,
      event_id: trackingRandomId('bw_e'),
      attribution_id: state && state.attributionId || '',
    }, payload);
    if (allowAnalytics && !pushEvent(eventName, eventPayload)) return false;
    if (allowAdvertising && typeof window.fbq === 'function') {
      window.fbq('trackCustom', eventName, Object.assign({}, eventPayload));
    }
    if (!allowAnalytics) return true;
    try {
      window.fetch(TRACK_ENDPOINT, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        ...
        body: JSON.stringify({
          ...
          pagePath: window.location.pathname,
          landingPage: state && state.landingPage || privacySafeTrackingUrl(window.location.href, allowAdvertising),
          firstPage: state && state.firstPage || window.location.pathname,
          visitorId: state && state.visitorId || '',
          sessionId: state && state.sessionId || '',
          attributionId: state && state.attributionId || '',
          referrer: document.referrer || '',
          utmSource: state && state.utm_source || params.get('utm_source') || '',
          utmMedium: state && state.utm_medium || params.get('utm_medium') || '',
          utmCampaign: state && state.utm_campaign || params.get('utm_campaign') || '',
          utmContent: state && state.utm_content || params.get('utm_content') || '',
          utmTerm: state && state.utm_term || params.get('utm_term') || '',
          utmId: state && state.utm_id || params.get('utm_id') || '',
          fbclid: allowAdvertising ? state && state.fbclid || params.get('fbclid') || '' : '',
          fbc: allowAdvertising ? state && state.fbc || params.get('fbc') || '' : '',
          fbp: allowAdvertising ? state && state.fbp || params.get('fbp') || '' : '',
          isPaid: Boolean(state && state.isPaid),
          screenWidth: String(window.screen && window.screen.width || ''),
  ...
  function boot() {
    bootAt = Date.now();
    startBlogEngagementPilot();
    if (isPostPage() && redesignEnabled()) {
      startRedesignC();
      return;
    }
    if (isPostPage()) {
  ...
  if (BLOG_JOURNEY_TEST_MODE) {
    window.__bwBlogJourneyTestHooks = {
      currentConsentPolicy: currentConsentPolicy,
      currentConsentState: currentConsentState,
      canonicalPaidTrackingState: canonicalPaidTrackingState,
