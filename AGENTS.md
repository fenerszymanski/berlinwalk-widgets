...
| `lead-form/` | Retired Berlin First-Day Survival Guide compatibility page. `index.html` has no email form and points visitors to the current history sampler; the old email/Velo assets remain historical reference only. Do not reactivate the capture flow. |
| `history-lead-magnet/` | `Berlin Before & Now` Custom Element in `inline` and `full` modes. Full mode keeps finite-series V2; compact inline mode uses V3 with one required/unchecked Story 2 + Story 3 consent and a second independent optional/unchecked newsletter V1 consent. The required choice never creates broad Wix marketing consent; the optional choice activates only after DOI, and prior unsubscribe needs a fresh optional choice plus fresh DOI. Includes the six licensed archive/current images, source manifest, tests, and the canonical Wix hourly Story 3 bridge under `velo/`. The guarded blog A/B loader is `js/lead-form-inject.js`; the existing live experiment remains pinned to `f985d8f5288df84d46e904298ad9186236561df1`, while the published SEO-support post uses `cf479a0457c271f826f93a03eea12cc7785dcd6e` for standalone attribution and its compact Mehringplatz embed. |
| `blog-home/` | Homepage Custom Element for the editorial blog teaser. `blog-home-element.js` defines `<bw-blog-home>` with the `Berlin Travel Notes` layout: one featured practical guide, optional compact `miniPosts` under the featured card, three right-rail note cards, image-led design, and a CTA to `/blog`. `data.json` is the curated mini-CMS for the homepage blog section. |
| `blog-index/` | Full `/blog` editorial hub Custom Element. `blog-index-element.js` defines `<bw-blog-index>` with a magazine-style lead story, secondary story rail, start-here links, search, topic shelves, free tools strip, and booking CTA. `data.json` is generated from live Wix Blog posts by `scripts/generate-blog-index-data.mjs`. |
...
| `paid-landing/` | Paid-traffic booking landing Custom Element. `paid-landing-element.js` defines `<bw-paid-landing>` with mini trust strip, image-led hero, live `<bw-booking-calendar>`, trust band, route/guide/FAQ sections, final CTA, sticky mobile CTA, and paid funnel tracking. It auto-loads `booking-calendar/booking-calendar-element.js`, so Wix only needs the tag and script URL. Intended for a normal Wix landing page with global header/footer hidden. |
| `site-header/` | Global site header Custom Element. `site-header-element.js` defines `<bw-site-header>` with a green top mini-bar (FreeTour rating + Free/Tip-based + Daily 11:30), main bar (logo + nav including `Games` to `/games/berlin-battle`, Resources dropdown + green BOOK NOW pill), shrink-on-scroll behavior (top bar collapses, logo + padding shrink past 32px), scroll progress bar (green→lime→yellow gradient), and a full-screen compact/mobile overlay with body scroll lock, Escape-to-close, and yellow CTA. `index.html` is the standalone GitHub Pages preview with a tall stage for scroll testing. |
| `site-footer/` | Global site footer Custom Element. `site-footer-element.js` defines `<bw-site-footer>` with a booking CTA, meeting point link, route summary, `Play` → `Berlin Battle`, planning/blog links, and partner-facing `Embed Berlin Tools` link. `index.html` is the standalone GitHub Pages preview/fallback. |
| `games-page/` | `/games` hub Custom Element prototypes. `games-page-element.js` defines `<bw-games-page>` with a full-bleed game hero, Spotlight Grid cards for Berlin Battle / Berghain Bouncer / Berlin Smile Challenge, status-based game board, booking bridge, and a sticky mobile choose-game bar. `games-hub-modal-element.js` defines alternate `<bw-games-hub-modal>` with filters, mode list, selected-game detail panel, rules, and CTAs. Do not point the global header to `/games` until the Wix page exists and is published. |
...
| `ultimate-berlin-trip-planner/` | BerlinTools standalone widget for pre-trip itinerary planning and tour conversion. `index.html` asks arrival date, trip length, arrival time/point, stay area, group type, first-time status, interests, trip mode, must-handle filters, pace, and tour intent, then returns a 1-7 day deterministic plan with Open-Meteo or monthly weather fallback, Sunday/holiday/Monday logic, visual plan board, Google Maps anchors, Berlin essentials, and lead-gated full plan/PDF/print. Velo source in `velo/` saves leads to `TripPlannerLeads`, sends instant + 7/3/1/day-of pre-booking emails, and suppresses future Ultimate reminders when a booking marker arrives because the existing booking email sequence handles booked guests. Launch handoff lives in `LAUNCH_RUNBOOK.md`; local status dashboard is `LAUNCH_CONTROL_ROOM.html`; generated verdict is `LAUNCH_STATUS.md`; `TRIGGERED_EMAIL_API_NOTES.md` records why first template creation is manual; the five real Triggered Email IDs are applied locally and the next step is Velo publish/live smoke; protected post-smoke visibility flip is `release-visibility.mjs`. |
| `berlin-first-day-rescue-plan/` | Paid instant product widget for the EUR 9 Berlin First-Day Rescue Plan. A temporary USD 1 checkout was used only for the 2026-07-02 live payment test. `index.html` renders the mobile-first scenario/intake/payment/plan flow against the root Content App `/api/rescue-plan` endpoint; public mock checkout is blocked. `email/` contains the single paste-ready backup Triggered Email and setup checklist; `velo/` contains the Wix HTTP webhook kit for sending that backup link after payment. Do not activate Meta delivery until the paused campaign and live product are checked in Ads Manager. |
...
| `blog-drafts/` | Draft blog posts before they are moved into Wix. Current lead-support package `berlin-then-and-now/` is also present in Wix as fully prepared draft `29816537-5ec8-49f2-ada9-3385ada5daa9`; it must remain `UNPUBLISHED` until Yusuf explicitly approves that exact draft. |
| `tools-hub/` | The `/tools` directory grid. `data.json` is the **single source of truth** for every embeddable BerlinWalk widget: slug, title, lead, legacy `category`, BerlinTools `hubCategory`, `type`, image, **`widgetUrl`**, **`embedHeight`**. New widgets must be added here. |
...
- `berlinwalk.com/berlin-before-and-now` — live `Berlin Before & Now` lead magnet pilot; hidden from navigation and `noindex,follow`
- `https://app.berlinwalk.com/api/history-lead` — DOI, story access/click, unsubscribe, and due-email API
- `https://www.berlinwalk.com/_functions/subscribe` — retired legacy endpoint; intentionally returns HTTP `410 lead_magnet_retired`
...
- **Wix Blog editor paragraph spacing changed after the blog redesign.** Do not add manual blank spacer paragraphs to new posts by default. Legacy posts may still contain spacer nodes, and `blog-journey-inject.js` normalizes those on public post pages. Only reintroduce a spacing workaround after current Wix editor/live QA proves a specific post is too tight.
...
3. **Monitor the Berlin Before & Now V2 pilot.** Safety ramp began `2026-07-17T05:50:19Z`: 10% variant for 24 hours, then automatic 50/50, across the six approved history slugs recorded in root `PROJECT_MEMORY.md` (old town, wide streets, Berlin Wall map, Ampelmann, Unter den Linden, founding year). Keep Alexanderplatz excluded unless a later explicit expansion is approved. Primary decision is no earlier than day 28 and 500 analytics-consented variant views. Run the root aggregate-only report with `node scripts/history-lead-magnet-report.mjs`; never print lead emails. `?bwHistoryLead=0` is the immediate per-request rollback; the global experiment config is the kill switch.
4. **Keep Story 3 delivery and both consent scopes healthy.** Wix `processHistoryLeadDueEmails` runs at `20 * * * *` and the daily Vercel cron is fallback only. Check DOI, finite-series and newsletter unsubscribe suppression, canonical booking branch, delivery failure/complaint rates, retention and no-PII redirects during readouts. Outlook delivery was verified on both the existing-subscriber and fresh-DOI paths. Never infer newsletter permission from the required finite-series checkbox: only the separate optional V1 choice, followed by DOI, may activate ongoing Wix marketing consent. The live Privacy addendum is the pragmatic operating text, not a claim of formal lawyer approval.
5. **Monitor the published SEO support post.** `/post/berlin-then-and-now` is live, present in the 226-post `/blog` inventory and sitemap, and its Search Console indexing request was accepted on 2026-07-17. Use consented lead/DOI evidence rather than page views as the outcome metric. Social promotion remains deferred by Yusuf.
6. **Publish the blog redesign v1.** Push `berlinwalk-widgets`, wait for GitHub Pages, then use `blog-index/blog-index-element.js?v=2` for `<bw-blog-index>` and add `js/blog-journey-inject.js?v=7` as a blog/post Custom Code helper.
7. **Live-test the reusable booking calendar.** Use `booking-calendar/` to create a Wix custom Booking Calendar POC, verify the current availability API path (`availabilityCalendar.queryAvailability()` vs Time Slots V2), bind real availability, and verify the exact native Booking Form query params for selected slot + guest count before replacing any live booking calendar.
