...
| `history-lead-magnet/` | `Berlin Before & Now` Custom Element in `inline` and `full` modes. V2 is a finite Story 2 + Story 3 series, not an open-ended newsletter: one required/unchecked checkbox, DOI where required, scope-limited unsubscribe, no silent broad Wix marketing subscription. Includes the six licensed archive/current images, source manifest, tests, and the canonical Wix hourly Story 3 bridge under `velo/`. The guarded blog A/B loader is `js/lead-form-inject.js`; final contrast release ref is `b32470b93390e9c0af664a27487cc812c2231d4b`. |
...
4. **Keep finite Story 3 delivery and consent evidence healthy.** Wix `processHistoryLeadDueEmails` runs at `20 * * * *` and the daily Vercel cron is fallback only. Check DOI, finite-series unsubscribe suppression, canonical booking branch, delivery failure/complaint rates, retention and no-PII redirects during readouts. Outlook delivery was verified on both the existing-subscriber and fresh-DOI paths. Do not broaden this consent into a newsletter; future marketing needs a separate later opt-in. The live Privacy addendum is the pragmatic operating text, not a claim of formal lawyer approval.
5. **Review, do not publish, the SEO support draft.** Wix draft `29816537-5ec8-49f2-ada9-3385ada5daa9` (`berlin-then-and-now`) is fully prepared and `UNPUBLISHED`. Do not publish it, regenerate `/blog`, request indexing, or launch social promotion without Yusuf's explicit approval for this exact draft.
