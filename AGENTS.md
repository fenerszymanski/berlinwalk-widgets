...
| `lead-form/` | Berlin First-Day Survival Guide email capture iframe widget. `lead-form/email/` is the source-of-truth for the Wix welcome email copy, preview, and paste-ready HTML block. Backend/internal offer keys may still say `berlin-survival-map` for compatibility. |
...
| `berlin-first-day-rescue-plan/` | Paid instant product widget for the Berlin First-Day Rescue Plan. Current live test price is USD 1 to keep Stripe test cost low; original target price was EUR 9. `index.html` renders the mobile-first scenario/intake/payment/plan flow against the root Content App `/api/rescue-plan` endpoint; public mock checkout is blocked. `email/` contains the single paste-ready backup Triggered Email and setup checklist; `velo/` contains the Wix HTTP webhook kit for sending that backup link after payment. Do not create Meta campaigns for this product until real Stripe checkout, post-payment plan opening, and backup email delivery are verified. |
...
- `https://www.berlinwalk.com/_functions/subscribe` — Berlin First-Day Survival Guide lead form; backend compatibility keys still use the old Survival Map naming
...
4. **DONE 2026-06-07: Email Marketing subscription helper installed.** Root cause was a hardcoded placeholder `WIX_API_KEY` const in live `http-functions.js`. Fixed: added `Backend/emailMarketingSubscription.js` (reads existing `berlinwalk-wix-api-key` secret), `post_subscribe` now calls `subscribeEmailMarketing(email)`, published, live-tested `subscribed:true, subscriptionDebug:"SUBSCRIBED"`. New leads now auto-subscribe. Optional leftover: remove the now-unused inline `subscribeViaRestApi` + placeholder const.
