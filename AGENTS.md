...
| `history-lead-magnet/` | `Berlin Before & Now` Custom Element in `inline` and `full` modes. Includes the six licensed archive/current images, source manifest, tests, and the canonical Wix hourly Story 3 bridge under `velo/`. The blog A/B loader is `js/lead-form-inject.js`. |
...
3. **Monitor the Berlin Before & Now pilot.** Safety ramp began `2026-07-17T05:50:19Z`: 10% variant on the old-town article for 24 hours, then automatic 50/50 on old-town + wide-streets. Keep Alexanderplatz excluded unless the day-14 volume rule is met and guardrails are clean. Primary decision is no earlier than day 28 and 500 analytics-consented variant views. `?bwHistoryLead=0` is the immediate per-request rollback; the global experiment config is the kill switch.
4. **Keep Story 3 delivery and pre-scale compliance healthy.** Wix `processHistoryLeadDueEmails` runs at `20 * * * *` and the daily Vercel cron is fallback only. Check DOI, unsubscribe suppression, canonical booking branch, delivery failure/complaint rates, and no-PII redirects during readouts. Obtain final legal wording review and a real Outlook-inbox delivery check before scaling.
5. **Publish the blog redesign v1.** Push `berlinwalk-widgets`, wait for GitHub Pages, then use `blog-index/blog-index-element.js?v=2` for `<bw-blog-index>` and add `js/blog-journey-inject.js?v=7` as a blog/post Custom Code helper.
6. **Live-test the reusable booking calendar.** Use `booking-calendar/` to create a Wix custom Booking Calendar POC, verify the current availability API path (`availabilityCalendar.queryAvailability()` vs Time Slots V2), bind real availability, and verify the exact native Booking Form query params for selected slot + guest count before replacing any live booking calendar.
