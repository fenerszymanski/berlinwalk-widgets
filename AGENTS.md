...
| `ultimate-berlin-trip-planner/` | BerlinTools standalone widget for pre-trip itinerary planning and tour conversion. `index.html` asks arrival date, trip length, arrival time/point, stay area, group type, first-time status, interests, pace, and tour intent, then returns a 1-7 day deterministic plan with Open-Meteo or monthly weather fallback, Sunday/holiday/Monday logic, a useful preview, and lead-gated full plan/PDF/print. Velo source in `velo/` saves leads to `TripPlannerLeads`, sends instant + 7/3/1/day-of emails, and switches to booked/prep email IDs when a booking marker arrives. |
