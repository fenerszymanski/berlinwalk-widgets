...
| `games-page/` | `/games` hub Custom Element. `games-page-element.js` defines `<bw-games-page>` with a full-bleed three-game hero, Spotlight Grid cards for Berlin Battle / Berghain Bouncer / Berlin Smile Challenge, status-based game board, FAQ, booking bridge, and a sticky mobile choose-game bar. Do not point the global header to `/games` until the Wix page exists and is published. |
