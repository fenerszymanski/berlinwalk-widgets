...
- `quick-summary/` - reusable quick summary and optional audio player. `data.json` is the canonical aggregate; `data/<slug>.json` files are generated page-level runtime payloads.
...
- `blog-index/` - `bw-blog-index` full `/blog` editorial hub Custom Element. `data.json` remains the canonical generated snapshot; the initial page uses the smaller `index.json`, while search/related-guide journeys lazy-load `archive.json`.
...
- `tools-hub/` - `bw-tools-hub` tools hub Custom Element. Wix Media icons are requested as 160/320px responsive AVIF renditions with browser fallback.
...
- `faq/` - `bw-faq` homepage FAQ Custom Element, plus existing iframe FAQ files for blog posts. `data.json` and `slug-map.json` are canonical; `data/<slug>.json` and the small `inject.js` loader are generated by `scripts/generate-faq-inject.mjs`.
...
- Wix MCP may help with CMS, blog posts, SEO metadata, Custom Embeds, site
  ...
  setup are manual.
- `faq/faq-element.js` and `faq/index.html` first request the page-level
  `faq/data/<slug>.json` shard and fall back to the canonical aggregates for
  backward compatibility. The small `faq/inject.js` loader follows the same
  pattern and emits FAQPage JSON-LD. Keep the iframe and injector files intact
  for blog posts. Regenerate FAQ and Quick Summary shards with
  `node scripts/generate-faq-inject.mjs` whenever their canonical data changes.
...
- `js/cta-inject.js` - deprecated one-time cleanup shim that removes old global tour CTA banners if an old Wix Custom Code URL still loads it. Remove the Wix Custom Code entry during a controlled Wix publish; the shim no longer watches every DOM mutation.
...
- `js/blog-journey-inject.js` - lightly polishes post body typography, removes
  ...
  each post. It lazy-loads `blog-index/archive.json`, while the index/sidebar
  path shares a cached request to `blog-index/index.json`; it links to related guides, relevant
