...
Yusuf requested an additional bike-lane interactive map widget for the same draft. `berlin-bike-lane-map/` was added as a supporting article embed, not as a separate BerlinTools card. It uses Leaflet with curated tourist street situations and the known Wix iframe Leaflet safeguards: no SRI attributes, scoped fallback Leaflet positioning CSS, and repeated `invalidateSize()` calls.
