...
const CATEGORY_ID = 'bf46ccbb-3db8-499a-95fe-4623868d6e1a';
const SLUG = 'how-to-read-a-berlin-address';
const TOOL_SLUG = 'berlin-address-compass';
const TITLE = 'How to Read a Berlin Address: Street Names, House Numbers and the Clues That Get You There';
const DRAFT_DIR = 'blog-drafts/how-to-read-a-berlin-address';
const BODY_PATH = path.join(ROOT, DRAFT_DIR, 'how-to-read-a-berlin-address.body.md');
const FAQ_PATH = path.join(ROOT, 'faq/data.json');
const EMBED_VERSION = '20260818ac1';
const EMBEDS = {
  '{{quick-summary}}': {
    id: 'quick_summary_how_to_read_a_berlin_address',
    url: 'https://fenerszymanski.github.io/berlinwalk-widgets/quick-summary/?post=how-to-read-a-berlin-address&v=' + EMBED_VERSION,
    height: '1120',
  },
  '{{widget:berlin-address-compass}}': {
    id: 'widget_berlin_address_compass',
    url: 'https://fenerszymanski.github.io/berlinwalk-widgets/berlin-address-compass/?v=' + EMBED_VERSION,
    height: '1600',
  },
  '{{faq}}': {
    id: 'faq_how_to_read_a_berlin_address',
    url: 'https://fenerszymanski.github.io/berlinwalk-widgets/faq/?post=how-to-read-a-berlin-address&v=' + EMBED_VERSION,
    height: '1360',
  },
};
...
const POST = {
  title: TITLE,
  slug: SLUG,
  seoTitle: 'How to Read a Berlin Address: Street, Number & Postcode',
  description: 'A calm guide to Berlin street names, house numbers, postcodes and entrance clues, plus a quick tool to check an address before you set off.',
  excerpt: 'Keep a Berlin address complete: street name, exact number, postcode and the entrance note that gets you through the right door.',
  socialTitle: 'How to Read a Berlin Address Without Getting Lost at the Door',
  socialDescription: 'Street, number, postcode and the small entrance clue that makes a Berlin address actually useful.',
  hashtags: ['berlin', 'berlinwalk', 'berlinaddress', 'berlintips', 'germanlanguage'],
  keywords: ['how to read a Berlin address', 'Berlin address format', 'Berlin house number', 'Berlin postcode', 'Berlin street names'],
  coverPath: 'images/optimized/01-address-cover.jpg',
};
...
async function main() {
  ...
  if (report.status !== 'UNPUBLISHED' || report.seoSlug !== SLUG || report.images !== 4 || report.imageAlts !== 4 || report.embeds.length !== 3 || report.bodyH1 !== 0 || report.captions !== 4 || report.embeds.some((item) => item.hasSpoiler)) {
