...
- Fixed the copy-kit behind-scroll issue Yusuf caught in the browser. The `Message ID JSON` panel is no longer sticky, the one-page checklist textarea is contained at `220px`, a small email-card guide appears before the card grid, and `Next missing template` closes the checklist before scrolling. Browser QA confirmed `jsonPosition: relative`, open checklist contained, next-card jump closes it, overflow `0`, and launch audit still reports `131 pass, 1 warn, 1 block`.
- Corrected the booking-aware email strategy after Yusuf pointed out the existing site booking email sequence already handles guests after booking. Ultimate now generates/requires only 5 pre-booking planner/sales Triggered Emails, removes active booked-path email templates, and suppresses future Ultimate scheduled reminders when a lead is officially or self-reported booked. Browser QA confirmed the copy kit has 5 cards, `0/5` counters, no `_BOOKED` placeholders, the existing-booking-sequence warning, and overflow `0`. Fixture QA confirms booked leads do not receive Ultimate scheduled reminders while cancelled booking markers can re-enter the sales path.
