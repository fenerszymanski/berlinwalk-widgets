...
function geminiPrompt(input) {
  return [
    ...
    'Write as Yusuf speaking directly to the guest before the trip.',
    'Use simple English. Short sentences are better. It should sound like a local guide, not like software.',
    'Do not use a fixed template. Do not make every day sound the same.',
    'Do not list interest labels like "History, Berlin Wall / Cold War, Free / low budget". Turn them into normal travel language.',
    ...
    'dayStories must include exactly one item for every input.plan.days item. Each text must be one simple sentence, 14-28 words, using a real place, block, or tour detail from that day.',
