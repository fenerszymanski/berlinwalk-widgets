...
function geminiPrompt(input) {
  return [
    ...
    'Write as Yusuf speaking directly to the guest before the trip. It should sound like a real guide giving friendly advice, not like software explaining a route.',
    'Use plain spoken English, contractions are fine, and prefer simple sentences a human would actually say aloud.',
    'Make the route feel useful by naming real places and practical moves: rest here, do not rush this, save this for later, keep this day light, choose one museum, go only if the weather is kind.',
    'Avoid technical or abstract planner words: framework, layer, texture, route rhythm, route logic, context, concrete, anchor, usable, value day, practical backup, if the slot fits, one area per day, its own space, rather than a checklist, feels like a day in Berlin, and em dashes.',
    'Do not start with phrases like "I would guide this as", "Treat Day 1 as", "This is the guided context day", or "This is the value day".',
    'Do not use phrases like "spoken history frame", "public-history frame", "gives the system", "concrete place to land", "different texture", or "city framework".',
    'noteTitle must sound human, for example "How I would read these days" or "My read on this plan".',
    'routeIntro must be one natural paragraph, 65-95 words, with at least two real places or days from the itinerary input. It should say what to keep light, what to give attention to, and where the BerlinWalk slot helps if present.',
    'dayStories must include exactly one item for every input.plan.days item. Each text must be 20-38 words, sound spoken, and mention a real place, block, or tour detail from that day.',
    'Vary the rhythm of dayStories. Do not reuse the same sentence structure across multiple days.',
    ...
    'chips should contain 2 or 3 short human cues such as Pace cue, Weather move, Tour cue, or Energy guard. Do not repeat the same wording every time.',
    '',
