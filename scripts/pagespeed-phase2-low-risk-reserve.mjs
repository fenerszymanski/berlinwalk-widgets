...
const EMBED_NAME = 'BerlinWalk PageSpeed Phase 2 Low Risk Mobile Reserve';
const STYLE_ID = 'bw-pagespeed-phase2-low-risk-mobile-reserve';
const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const SOURCE_FILE = path.resolve(SCRIPT_DIR, '..', 'performance', 'phase2', 'batch-a-low-risk-mobile-reserve.html');
...
function parseArgs(argv) {
  ...
  if (snapshotIndex !== -1) {
    if (!argv[snapshotIndex + 1]) throw new Error('Missing path after --snapshot-out.');
    args.snapshotOut = path.resolve(argv[snapshotIndex + 1]);
  }
  return args;
}
...
function minifyStyle(source) {
  const style = source.match(/<style\b[^>]*>[\s\S]*?<\/style>/i)?.[0];
  if (!style) throw new Error('Source is missing its style element.');
  return style
    .replace(/<!--[\s\S]*?-->/g, '')
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/\s+/g, ' ')
...
function validateHtml(html) {
  const required = [
    ...
    'comp-mozc935g3',
    'comp-mrb6jar63',
    'bw-rewind-final-page',
    'min-height',
  ];
  const forbidden = ['<script', 'MutationObserver', 'appendChild', 'innerHTML', 'window.', 'location.', 'pointer-events', 'overflow:', 'display:none', 'height:0'];
  ...
  if (openBraces !== closeBraces) throw new Error(`Unbalanced CSS braces: ${openBraces} open, ${closeBraces} close.`);
...
function validateStateGates(embeds) {
  ...
  for (const gate of STATE_GATES) {
    ...
    if (!current) {
      problems.push(`${gate.name}: missing ${gate.id}`);
      continue;
    }
...
async function main() {
  ...
  const before = {
    ...
    minifiedLength: html.length,
    minifiedSha256: sha256(html),
  ...
  if (args.mode === 'dry-run' || args.mode === 'readback') {
    const matchesSource = current ? current.embedData?.html === html : false;
    console.log(JSON.stringify({ ok: true, ...before, matchesSource, headroom: MAX_EMBED_HTML_LENGTH - html.length }, null, 2));
    return;
  }
  ...
  if (JSON.stringify(before.protected) !== JSON.stringify(afterProtected)) {
    throw new Error('Protected booking/consent/analytics/legacy-reserve state changed during the batch. Stop and inspect before any further publish.');
  }
  ...
  console.log(JSON.stringify({
    ...
    rollback: `node scripts/pagespeed-phase2-low-risk-reserve.mjs --disable`,
  }, null, 2));
}
