...
const EMBED_NAME = 'BerlinWalk PageSpeed Phase 2 Commercial Template Reserve';
const STYLE_ID = 'bw-pagespeed-phase2-commercial-template-reserve';
const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const SOURCE_FILE = path.resolve(SCRIPT_DIR, '..', 'performance', 'phase2', 'batch-b-commercial-template-reserve.html');
...
function minifyHead(source) {
  const links = [...source.matchAll(/<link\b[^>]*>/gi)].map((match) => match[0].replace(/\s+/g, ' ').trim());
  ...
  const minifiedStyle = style
    ...
    .replace(/;}/g, '}')
    .trim();
  return [...links, minifiedStyle].join('');
}
...
function validateHtml(html) {
  const required = [
    ...
    'comp-mpw9b04c',
    'comp-mr7jtqiv',
    'bw-audio-route-products-frame-host',
    'bw-audio-route-products-frame',
    'min-height',
  ];
  const forbidden = [
    '<script',
  ...
  if ((html.match(/<link\b/gi) || []).length !== 2) throw new Error('Expected exactly two static preconnect links.');
...
async function main() {
  ...
  if (JSON.stringify(before.protected) !== JSON.stringify(afterProtected)) {
    throw new Error('Protected booking/consent/analytics/reserve state changed during the batch. Stop and inspect before any further publish.');
  }
  ...
  console.log(JSON.stringify({
    ...
    rollback: 'node scripts/pagespeed-phase2-commercial-reserve.mjs --disable',
  }, null, 2));
}
