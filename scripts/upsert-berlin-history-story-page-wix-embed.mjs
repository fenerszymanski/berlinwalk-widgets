...
function buildHtml() {
  ...
  const js = '(()=>{const P=' + JSON.stringify(p) + ';const clean=s=>(s||"/").replace(/\\/+$/,"" )||"/";if(clean(location.pathname)!==P.path)return;'
    ...
    + 'function layout(main,host){if(host.parentElement!==main)main.appendChild(host);host.style.setProperty("pointer-events","auto","important");if(main===document.body)return;host.style.setProperty("grid-row","2 / 3","important");host.style.setProperty("min-width","0","important");host.style.setProperty("width","100%","important");Array.from(main.children).forEach(child=>{if(child!==host)child.style.setProperty("display","none","important")})}'
